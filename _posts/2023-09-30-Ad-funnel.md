...
Hence, ad systems can't really be very successful by uniformly printing random ads on random ads opportunities but need smarter a process to decide if an ad opportunity is worth it, given a finite set of ads from your dear advertisers. The smarter process is usually a **ranking system**: given some context encoding the ad opportunity, it ranks all available ads from a finite set, with respect to some metric, and either push the first one, or sample one non uniformly using the metric used to rank.

## The advertising funnel

Now there is a distinction to make in the goals advertisers usually have: they either want to be known and assert their online presence, or they want to generate user engagement and capture traffic to their own websites. The first kind of objective usually isn't too demanding, in terms of ad system complexity that is, as advertisers will be satisfied as long as somebody actually sees the ad slot it bought. The second objective is the one that interest us here and is a more complex to efficiently pull off.

User engagement is not a binary quantity describing "interested" and "not interested" similar to a "seen", "not seen" that the first objective would be about, but instead it is a number of steps with a causal structure to it encoding a degree of interest of an user. In the advertising lingo, it's called a **funnel**, and in practice it is specific to what advertisers want.
...
Note that each step in the funnel is a binary event: either there is some degree of engagement (such as a click), unlocking the possibility to reach the next step, or there is not and it ends there.

## The classic approach

A very useful information for ad ranking systems is an estimate of the probability of occurrence of funnel events, such as clicks, visits and conversions, given an ad opportunity context. As funnel events are binary in nature, they can be modeled by Bernoulli trials with probability $p_{event}$ and because ad opportunities can be considered i.i.d, funnel events realizations $y_{event}$ are assumed independent.

A simple way of doing so is to build individual prediction models for every funnel events: one CTR model, one model predicting a visit rate and another one dedicated to conversions predictions. Then, our example funnel is then described by the triplet of independent models
...
is a learned mapping between i.i.d context vectors and funnel events probabilities (such as a deep neural network).

## Towards a multi-task learning paradigm

While it provides flexibility to a certain extent, a system of independent models approach has some drawbacks:
...
- Every models being independent, it's not possible to encode some special properties of the problem. In particular, there is no guarantee that $ P(click \mid \textbf{x}) \geq P(visit \mid \textbf{x}) \geq P(conversion \mid \textbf{x}) $ for the some input context $\textbf{x}$.
...
As our model still need a scalar loss function to minimize, a simple approach is to **scalarize** the multi-objective by finding a real valued function $g: \mathbb{R}^T \mapsto \mathbb{R}$. The simplest of such functions is a linear combination of the individual task losses:
...
The flexibility of combining multiple loss corresponding to multiple objective optimized jointly also allows to encode the causal nature of the funnel - that is, $P(click) \geq P(visit) \geq P(conversion)$. Indeed, we can easily transform this hard constraint into a soft penalty in the loss function in a similar fashion to what is usually done with weight decay to avoid overfitting in a deep learning model. This penalty terms increase the loss if the previous constraint is violated, and contributes nothing otherwise. In practice, it would make the model converge to a solution satisfying this additional constraint (at the cost of perhaps a harder convergence).

$$ \mathcal{L}_{causal} = \frac{1}{N} \sum_i [ \max( \hat{y}_{visit}^i - \hat{y}_{click}^i, 0) + \max( \hat{y}_{conversion}^i - \hat{y}\_{visit}^i, 0) ] $$

### Model architectures

While it seems to exist a whole taxonomy of architectures for multi-task learning, relevance of those definitely depends on the tasks they try solving, and in particular the modality of the input data. This post being about ad funnel, we probably deal with tabular data encoding an ad opportunity. In that case, about three kind of architectures seem to be widely used to solve the problem described above:
...
I implemented (in TF, sorry) the three simple architecture described above [here](https://github.com/clabrugere/multitask-learning/tree/main). They are not really optimized and use MLP for encoders as an example. Nevertheless it's flexible enough to be adapted without too much trouble.
